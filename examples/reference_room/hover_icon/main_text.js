...
function preload() {
    ...
    this.load.image("qmark", "../../assets/images/question_mark.png");
...
function create() {
    ...
    hintSprite = this.add.sprite(0, 0, "qmark");
    hintSprite.visible = false;

    this.input.on("pointerup", checkObjectSelection, this);
    this.input.on("pointermove", checkObjectHint, this);
}

function isPointOnObject(x, y, object) {
...
function checkObjectHint(pointer) {
    ...
    if (!selectedObject || !selectedObject.properties["description"]) {
        hintSprite.visible = false;
        return;
    }
...
function checkObjectSelection(pointer) {
    ...
    if (selectedObject) {
        ...
        let topPadding = 50;
        let leftPadding = 40;
        dialogBox = new Phaser.Geom.Rectangle(
            config.width - (config.width * 11) / 12,
            config.height - (config.height * 5) / 16,
            (config.width * 5) / 6,
            (config.width * 1) / 6
        );
        ...
        var text = this.add.text(
            dialogBox.x + leftPadding,
            dialogBox.y + topPadding,
